...
describe("EncryptedChannel", function() {
  ...
  beforeEach(function() {
    ...
    channel = new EncryptedChannel("private-encrypted-test", webpubsub, nacl);
  });
  ...
  describe("#handleEvent", function() {
    ...
    describe("on other events", function() {
      ...
      it("should emit webpubsub: prefixed events unmodified", function() {
        let payload = { test: "payload" };
        let boundCallback = jasmine.createSpy("boundCallback");
        channel.bind("webpubsub:subscription_error", boundCallback);
        channel.handleEvent({
          event: "webpubsub:subscription_error",
          data: payload
        });
        expect(boundCallback).toHaveBeenCalledWith(payload, {});
      });
