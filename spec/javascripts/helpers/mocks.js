...
var Mocks = {
    ...
    transport.connect = jasmine.createSpy('connect').and.callFake(function() {
      ...
      setTimeout(() => {
        transport.emit('message', {
          data: JSON.stringify({
            event: 'webpubsub:connection_established',
            data: {
              socket_id: '123.456',
  ...
  getWebpubsub: function(config) {
    var webpubsub = new EventsDispatcher();
    webpubsub.config = config;
    webpubsub.unsubscribe = jasmine.createSpy("unsubscribe");
    webpubsub.send_event = jasmine.createSpy("send_event");
    return webpubsub;
  },
